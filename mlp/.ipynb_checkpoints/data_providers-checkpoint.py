...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        """
        return np.eye(self.num_classes)[int_targets]
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        data = data[:,2:]  # remove first 2 columns
        data = data.flatten()  # flatten 
        data = data[data >= -99.8]  # filter out NaN data (-99.9)
        # normalise data to zero mean, unit standard deviation
        data -= data.mean()
        data /= data.std()
        # convert from flat sequence to windowed data
        data = np.resize(data,(math.ceil(data.size/window_size),window_size))  
        print(data.shape)
            # ! HACK does not work for data.size % window_size != 0 
