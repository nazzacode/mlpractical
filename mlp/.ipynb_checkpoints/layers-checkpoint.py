...
class DropoutLayer(StochasticLayer):
    ...
    def __init__(self, rng=None, incl_prob=0.5, share_across_batch=True):
        ...
        self.rng = rng if rng != None else np.random
        self.mask = None

        
    def make_mask(self, inputs_shape):
        """Return (and set) the mask of the dropout layer"""
        shape = inputs_shape[1:] if self.share_across_batch else inputs_shape
        self.mask = self.rng.uniform(low=0., high=1., size=shape) \
                     < self.incl_prob 
        return self.mask
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        """
        if stochastic:
            return inputs * self.make_mask(inputs.shape)
        else:
